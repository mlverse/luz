...
This example is an adaptation of the ‘Training a causal language model
from scratch’ class from the [Hugging Face NLP
course](https://huggingface.co/learn/nlp-course/chapter7/6?fw=pt).
...
pre-process it into a format that is suitable for training the model.

That means that we need to:

1.  Download data
2.  Train a tokenizer for this dataset
3.  Be able to produce sequences of tokens in the format expected by the
    model

We are going to use 2 datasets available in Hugging Face Hub. The first
contain all R packages source code available on CRAN. The second
...
We can finally implement the torch dataset that we are going to use for
training the model. We are going to use the
...
The main motivation is that we can’t really know the total number of
samples in the dataset, so we can implement a `.getitem()` method to get
any arbiratrary sample. Thus we implement the `.iter` method that
returns a new sample every time it’s called.
...
version of GPT2. We also define a `generate` method allowing us to
sample from the model given an initial context.
