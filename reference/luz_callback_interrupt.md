...
Adds a handler that allows interrupting the training loop using
`ctrl + C`. Also registers a `on_interrupt` breakpoint so users can
register callbacks to be run on training loop interruption.
