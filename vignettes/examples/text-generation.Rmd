...
read_datasets <- function() {
  sources <- "dfalbel/github-r-repos"#c("dfalbel/cran-packages")#, "dfalbel/github-r-repos")
  sources |>
    purrr::map_chr(
      \(x) hfhub::hub_snapshot(x, repo_type = "dataset", allow_patterns = "parquet$")
    ) |>
    fs::path("data") |>
    purrr::map(arrow::open_dataset) |>
    purrr::map(\(x) dplyr::select(x, content)) |>
    purrr::map(\(x) x |> dplyr::mutate(
      content = arrow::cast(content, arrow::string())
    )) |>
    purrr::reduce(dplyr::union_all) |>
    dplyr::filter(!is.na(content)) |> 
    # arrow::map_batches(function(batch) {
    #   content <- as.character(batch$content)
    #   nm <- purrr::map_chr(content, rlang::hash) |> paste0(".txt")
    #   paths <- file.path("data", nm)
    #   purrr::walk2(content, paths, \(x, f) readr::write_file(x, file = f))
  ...
  .iter = function() {
    ...
    function() {
      while (length(sequence) < (self$context_length + 1) && i <= nrow(self$data)) {
        sequence <<- c(
          sequence, 
          self$tok$encode(paste("<fbegin>", self$data$content[i], "<fend>"))$ids
        )
        i <- i + 1L
      }
...
net <- nn_module(
  ...
  forward = function(x) {
    self$gpt(x$view(c(1, -1)))$transpose(2,3)
  }
)
